...
case class FigletSmusher(
  ...
  def currentSmushAmount(buffer: Array[String], curChar: List[String], curCharWidth: Int, prevCharWidth: Int): Int = {
    ...
    for (row <- 0 until font.height) {
      ...
      if (Direction.right_to_left == direction) {
        val temp = lineLeft
        lineLeft = lineRight
        lineRight = temp
      }
      ...
      if (ch1.isEmpty || ch1 == " ") {
        amt += 1
      } else if (ch2.nonEmpty && smushChars(ch1, ch2, curCharWidth, prevCharWidth).nonEmpty) {
        amt += 1
      }
      if (amt < maxSmush) {
        maxSmush = amt
  ...
  private def smushChars(left: String = "", right: String = "", curCharWidth: Int, prevCharWidth: Int): String = {
    if (left.nonEmpty && left.trim.isEmpty) {
      return right
    } else if (right.nonEmpty && right.trim.isEmpty) {
      return left
    }
    ...
    if ((font.smushMode & SM_HARDBLANK) != 0 && left == font.hardBlank && right == font.hardBlank) {
      return left
    }
    ...
    smushes.foreach(entry => {
      if (entry._1.contains(left) && entry._2.contains(right)) {
        return right
      }
      if (entry._2.contains(left) && entry._1.contains(right)) {
        return left
      }
    })
    ...
    if ((font.smushMode & SM_PAIR) != 0) {
      List(left + right, right + left)
        .foreach(pair => if (PAIRS.contains(pair)) {
          return "|"
        })
    }
